(async function() {
  'use strict';
  var $, DATOM, DBay, FS, File_mirror, GUY, Interlex, PATH, Pipeline, Production_iterator, Prompt_file_reader, Prompt_parser, SQL, Syntax, Transformer, U, WG, alert, compose, debug, echo, end_of_line, get_types, help, hide, info, inspect, lets, log, new_datom, new_prompt_lexer, plain, praise, rpr, stamp, start_of_line, transforms, trash, types, urge, warn, whisper,
  ...
  */
    ...
    */
    ...
    class Prompt_file_reader {
      ...
      constructor(cmd, flags = null) {
        var f, insert_into, key, ref, ref1;
        debug('Ω___5', cmd, flags);
        hide(this, 'types', get_types());
        this.cfg = this.types.create.pfr_constructor_cfg(cmd, flags, null);
        this._file_mirror = new File_mirror(this.cfg.db_path, this.cfg.datasource_path, (ref = flags != null ? flags.trash_db : void 0) != null ? ref : false);
  ...
  ;`);
      ...
      //---------------------------------------------------------------------------------------------------------
      _populate_db() {
        return whisper('Ω___7', "Prompt_file_reader::_populate_db");
      }

      //---------------------------------------------------------------------------------------------------------
      * [Symbol.iterator]() {
        var all_rows, blank_line_count, i, len, line_count, nonmatching_line_count, read_prompt_count, record, ref, row, unsampled_line_count, written_prompt_count;
        ...
        all_rows = this._file_mirror._db.all_rows(SQL`select * from fm_datasources order by lnr;`);
        /* TAINT use API */        whisper('Ω__10', "Prompt_file_reader::_populate_db", GUY.trm.white(`read ${U.format_nr(all_rows.length)} lines from DB`));
//.......................................................................................................
/* NOTE this could / should be within a transaction but runs just as fast without one */
        for (i = 0, len = all_rows.length; i < len; i++) {
          ...
          if (modulo(line_count, 1e3) === 0) {
            whisper('Ω__11', "Prompt_file_reader::_populate_db", GUY.trm.white(`line count: ${U.format_nr(line_count, 8)}`));
          }
          //.....................................................................................................
          /* EXCLUDE EMPTY LINES */
          if (/^\s*$/.test(row.line)) {
          ...
          if (written_prompt_count >= this.cfg.flags.max_count) {
            whisper('Ω__12', "Prompt_file_reader::_populate_db", GUY.trm.white(`stopping because prompt count exceeds max prompt count of ${U.format_nr(this.cfg.flags.max_count)} prompts`));
            break;
          }
        }
        //.......................................................................................................
        ...
        //.......................................................................................................
        whisper('Ω__13');
        whisper('Ω__14', "Prompt_file_reader::_populate_db", GUY.trm.white(`line count:                    +${U.format_nr(line_count, 12)}`));
        //.......................................................................................................
        whisper('Ω__15', "Prompt_file_reader::_populate_db", GUY.trm.white(`blank line count:              –${U.format_nr(blank_line_count, 12)}`));
        //.......................................................................................................
        whisper('Ω__16', "Prompt_file_reader::_populate_db", GUY.trm.white(`'unsampled' line count:        –${U.format_nr(unsampled_line_count, 12)}`));
        //.......................................................................................................
        whisper('Ω__17', "Prompt_file_reader::_populate_db", GUY.trm.white(`non-pre-matching line count:   –${U.format_nr(nonmatching_line_count, 12)}`));
        //.......................................................................................................
        whisper('Ω__18', "Prompt_file_reader::_populate_db", GUY.trm.white(`non-matching prompt count:     –${U.format_nr(this._prompt_parser.state.counts.non_matches, 12)}`));
        // #.......................................................................................................
    ...
    Prompt_file_reader.insert_into = {
      //.......................................................................................................
      fm_datasources: function(d) {
        /* TAINT validate? */
        return this._db.alt(this._insert_into.fm_datasources, d);
      },
      //.......................................................................................................
   ...
   */
  //===========================================================================================================
  Production_iterator = class Production_iterator {
    //---------------------------------------------------------------------------------------------------------
  ...
  */
  ...
  if (module === require.main) {
    await (() => {
      ...
      ref = new Prompt_file_reader(cmd, flags);
      for (d of ref) {
        debug('Ω__20', d);
      }
      return;
