  ...
  foreign key ( prompt_id ) references prompts ( id ) );`);
      ...
      _populate_db() {
        ...
        this._db(() => {
          ...
          for (row of ref) {
            help('Ω__13', this._pipeline.send(row));
      ...
      parse_first_records(source) {
        var ref;
        return (ref = (this.parse_all_records(source))[0]) != null ? ref : [];
      }

      //---------------------------------------------------------------------------------------------------------
      parse_all_tokens(source) {
        var R, eol, line, lnr, ref, ref1, token, tokens, x;
        R = [];
        ref = GUY.str.walk_lines_with_positions(source);
        for (x of ref) {
          ({lnr, line, eol} = x);
          ...
          for (token of ref1) {
            ...
            tokens.push(this._prompt_parser._cast_token(lnr, token));
          }
        }
        return R;
      }
      ...
      parse_first_tokens(source) {
        var ref;
        return (ref = (this.parse_all_tokens(source))[0]) != null ? ref : [];
      }
