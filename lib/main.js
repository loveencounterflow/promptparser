(async function() {
  ...
  */
  ...
  ;`);
      ...
      //---------------------------------------------------------------------------------------------------------
      _populate_db() {
        var read_prompt_count, unique_row_ids, written_prompt_count;
        ...
        this._db(() => {
          ...
          ref = this._db(SQL`select * from datasources where line != '' order by lnr;`);
          for (row of ref) {
            // @cfg.flags.match
            // @cfg.flags.sample
            this./* TAINT use API */_pipeline.send(row);
            ...
            for (record of ref1) {
              ...
              if (record.table === 'prompts') {
                read_prompt_count++;
                if (modulo(read_prompt_count, 1e3) === 0) {
                  whisper('Ω__11', "Prompt_file_reader::_populate_db", GUY.trm.white(`${format_nr(read_prompt_count)}`));
                }
              }
              //.................................................................................................
            ...
            if (written_prompt_count >= this.cfg.flags.max_count) {
              whisper('Ω__12', "Prompt_file_reader::_populate_db", GUY.trm.white(`stopping because prompt count exceeds \`--max-count\` (${format_nr(this.cfg.flags.max_count)})`));
              break;
            }
          }
          return null;
        ...
        /* TAINT use API */        whisper('Ω__13', "Prompt_file_reader::_populate_db", GUY.trm.white(`inserted ${format_nr(written_prompt_count)} rows into DB at ${this.cfg.db_path}`));
        //.......................................................................................................
        return null;
      }
