(async function() {
  'use strict';
  var $, DATOM, DBay, FS, File_mirror, GUY, Interlex, PATH, Pipeline, Prompt_file_reader, Prompt_parser, SQL, Syntax, Transformer, U, alert, build_file_db, compose, debug, demo_file_as_virtual_table, echo, end_of_line, get_types, help, hide, info, inspect, lets, log, new_datom, new_prompt_lexer, plain, praise, rpr, stamp, start_of_line, transforms, urge, warn, whisper,
  ...
  */
  ...
  foreign key ( prompt_id ) references prompts ( id ) );`);
          hide(this, '_insert_into', {
            datasources: this._db.prepare_insert({
              into: 'datasources'
            }),
            prompts: this._db.prepare_insert({
              into: 'prompts',
              on_conflict: {
                update: true
              }
            }),
            generations: this._db.prepare_insert({
              into: 'generations'
            })
          });
          return null;
    ...
    //---------------------------------------------------------------------------------------------------------
    File_mirror.required_table_names = ['prompts'];
    ...
    File_mirror.insert_into = {
      ...
      datasources: function(d) {
        /* TAINT validate? */
        return this._db(this._insert_into.datasources, d);
      },
      //.......................................................................................................
      prompts: function(d) {
        /* TAINT validate? */
        return this._db(this._insert_into.prompts, lets(d, function(d) {
      ...
      generations: function(d) {
        /* TAINT validate? */
        return this._db(this._insert_into.generations, d);
      }
    };
  ...
  */
  ...
  Prompt_file_reader = class Prompt_file_reader extends File_mirror {
    ...
    parse(source) {
      ...
      for (i = 0, len = R.length; i < len; i++) {
        ...
        this._db(this._insert_into[d.table], d.fields);
      }
  ...
  demo_file_as_virtual_table = function() {
    var db, record, ref, ref1, row;
    db = new Prompt_file_reader('/dev/shm/prompts-and-generations.sqlite');
    ref = db._db.all_rows(SQL`select * from datasources order by lnr;`);
    for (row of ref) {
      debug('Ω__18', row);
