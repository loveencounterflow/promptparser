(async function() {
  'use strict';
  var $, DATOM, DBay, FS, File_mirror, GUY, Interlex, PATH, Pipeline, Prompt_file_reader, Prompt_parser, SQL, Syntax, Transformer, U, WG, alert, build_file_db, compose, debug, echo, end_of_line, get_types, help, hide, info, inspect, lets, log, new_datom, new_prompt_lexer, plain, praise, rpr, stamp, start_of_line, transforms, trash, types, urge, warn, whisper,
  ...
  */
    ...
    class File_mirror {
      ...
      _create_db_structure_if_necessary() {
        if (U.db_has_all_table_names(this._db, this.constructor.required_table_names)) {
          whisper('Ω___6', `File_mirror::_create_db_structure_if_necessary: re-using DB at ${this.cfg.db_path}`);
        } else {
          whisper('Ω___7', `File_mirror::_create_db_structure_if_necessary: creating structure of DB at ${this.cfg.db_path}`);
      ...
      _populate_db() {
        ...
        try {
          ...
          if ((ref = error.code) === 'ENOENT' || ref === 'EACCES' || ref === 'EPERM') {
            whisper('Ω__10', "File_mirror::_populate_db", U.color.bad(error.message));
            process.exit(111);
          }
          throw error;
        }
  ...
  ;`);
      ...
      //---------------------------------------------------------------------------------------------------------
      _populate_db() {
        var blank_line_count, line_count, nonmatching_line_count, read_prompt_count, unique_row_ids, unsampled_line_count, written_prompt_count;
        ...
        //.......................................................................................................
        this._db(() => {
          var record, ref, ref1, row, row_id;
          ...
          for (row of ref) {
            /* TAINT use API */            line_count++;
            if (modulo(line_count, 1e3) === 0) {
              whisper('Ω__13', "Prompt_file_reader::_populate_db", GUY.trm.white(`line count: ${U.format_nr(line_count, 8)}`));
            }
            //...................................................................................................
            /* EXCLUDE EMPTY LINES */
            if (/^\s*$/.test(row.line)) {
            ...
            //...................................................................................................
            /* --MATCH */
            if (this.cfg.flags.match != null) {
              this.cfg.flags.match.lastIndex = 0/* TAINT ensure when constructing match that lastIndex is never used */
              if (!this.cfg.flags.match.test(row.line)) {
            ...
            if (written_prompt_count >= this.cfg.flags.max_count) {
              whisper('Ω__14', "Prompt_file_reader::_populate_db", GUY.trm.white(`stopping because prompt count exceeds max prompt count of ${U.format_nr(this.cfg.flags.max_count)} prompts`));
              break;
            }
          }
          return null;
        ...
        //.......................................................................................................
        written_prompt_count = this._db.single_value(SQL`select count(*) from prompts;`);
        //.......................................................................................................
        /* TAINT use API */        whisper('Ω__15');
        whisper('Ω__16', "Prompt_file_reader::_populate_db", GUY.trm.white(`line count:              +${U.format_nr(line_count, 12)}`));
        //.......................................................................................................
        if (blank_line_count > 0) {
          whisper('Ω__17', "Prompt_file_reader::_populate_db", GUY.trm.white(`blank line count:        –${U.format_nr(blank_line_count, 12)}`));
        }
        //.......................................................................................................
        if (unsampled_line_count > 0) {
          whisper('Ω__18', "Prompt_file_reader::_populate_db", GUY.trm.white(`'unsampled' line count:  –${U.format_nr(unsampled_line_count, 12)}`));
        }
        //.......................................................................................................
        if (nonmatching_line_count > 0) {
          whisper('Ω__19', "Prompt_file_reader::_populate_db", GUY.trm.white(`non-matching line count: –${U.format_nr(nonmatching_line_count, 12)}`));
        }
        //.......................................................................................................
        whisper('Ω__20', "Prompt_file_reader::_populate_db", GUY.trm.white(`inserted ${U.format_nr(written_prompt_count)} rows into DB at ${this.cfg.db_path}`));
        //.......................................................................................................
        return null;
      }
