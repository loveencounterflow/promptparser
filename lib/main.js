(async function() {
  'use strict';
  var DATOM, DBay, GUY, Image_walker, Journal_walker, Prompt_db, SQL, alert, cmd, debug, echo, flags, get_types, help, hide, info, inspect, known_path_ids, lets, lines, log, new_datom, plain, pluck, praise, reverse, rpr, run_image_walker, run_journal_walker, stamp, urge, warn, whisper;
  ...
  //===========================================================================================================
  run_journal_walker = function*(prompt_db) {
  ...
  //===========================================================================================================
  run_image_walker = function*(prompt_db) {
    known_path_ids = prompt_db.img_get_known_path_ids();
    yield* new Image_walker({cmd, flags, known_path_ids});
    return null;
  };
  ...
  if (module === require.main) {
    await (() => {
      ...
      (() => {
        var count, d, ref;
        count = 0;
        ref = run_image_walker(prompt_db);
        for (d of ref) {
          count++;
      ...
      (() => {
        var d, ref;
        ref = run_journal_walker(prompt_db);
        for (d of ref) {
          prompt_db.insert_into[d.table](d.fields);
        }
        return null;
