(async function() {
    ...
    class File_mirror {
      ...
      _create_db_structure_if_necessary() {
        if (U.db_has_all_table_names(this._db, this.constructor.required_table_names)) {
          help(`Ω___7 re-using DB at ${this.cfg.path}`);
        } else {
          warn(`Ω___8 creating structure of DB at ${this.cfg.path}`);
  ...
  */
  ...
  Prompt_file_reader = class Prompt_file_reader extends File_mirror {
    ...
    //     # @_db =>
    //     info 'Ω__14', GUY.trm.reverse GUY.trm.bold GUY.trm.gold d
    //     @_db @_insert_into[ d.table ], d.fields
    //   # debug 'Ω__15', ( "#{t.$key}#{rpr t.value}" for t in R ).join '|'
    //   return R
  ...
  demo_file_as_virtual_table = function() {
    ...
    db_read = new DBay({
      path: '/dev/shm/prompts-and-generations.sqlite'
    });
    db._db(() => {
