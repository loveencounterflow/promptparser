(async function() {
  ...
  */
  ...
  Prompt_parser = class Prompt_parser extends Transformer {
    ...
    $lex() {
      return (row, send) => {
        ...
        for (lexeme of ref) {
          ...
          if (!this.types.isa.symbol(lexeme)) {
            lexeme = lets(lexeme, function(lexeme) {
              return lexeme.lnr1 = lexeme.lnr2 = row.lnr;
            });
          }
          send(lexeme);
        }
    ...
    $show() {
      return (d) => {
        // urge 'Ω___8', rpr d # if d.$key is 'generation'
        return null;
      };
    }
    ...
    */
    ...
    class File_mirror {
      ...
      constructor(db_path, datasource_path) {
        hide(this, 'types', get_types());
        this.cfg = this.types.create.fm_constructor_cfg(db_path, datasource_path);
      ...
      _prepare_db_connection() {
        // whisper "Ω__10 File_mirror._prepare_db_connection"
        // @_db =>
      ...
      _create_db_structure_if_necessary() {
        if (U.db_has_all_table_names(this._db, this.constructor.required_table_names)) {
          help(`Ω__11 re-using DB at ${this.cfg.path}`);
        } else {
          warn(`Ω__12 creating structure of DB at ${this.cfg.path}`);
      ...
      _create_db_structure() {
        whisper("Ω__13 File_mirror::_create_db_structure");
        this._clear_db();
        this._db(() => {
    ...
    */
    ...
    class Prompt_file_reader extends File_mirror {
      ...
      _create_db_structure() {
        ...
        whisper("Ω__14 Prompt_file_reader::_create_db_structure");
        this._db(() => {
  ...
  foreign key ( prompt_id ) references prompts ( id ) );`);
      ...
      //-----------------------------------------------------------------------------------------------------------
      _populate_db() {
        whisper("Ω__15 Prompt_file_reader::_populate_db()");
        super._populate_db();
        this._db(() => {
          ...
          results = [];
          for (row of ref) {
            help('Ω__17', this._pipeline.send(row));
            results.push((function() {
              ...
              for (record of ref1) {
                info('Ω__18', record);
                results1.push(this.insert_into[record.table](record.fields));
              }
              return results1;
            }).call(this));
          }
        ...
        //     debug 'Ω__19', row
        //     help 'Ω__20', @_pipeline.send row.line
