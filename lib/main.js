(async function() {
  'use strict';
  var DBay, GUY, Image_walker, Journal_walker, Prompt_db, SQL, U, alert, debug, echo, get_types, help, hide, info, inspect, log, plain, pluck, praise, reverse, rpr, trash, urge, warn, whisper;
  ...
  join jnl_densities  as d on ( p.id = d.prompt_id );`);
        ...
        return this.db(SQL`create view rowcounts as
  ...
  if (module === require.main) {
    await (() => {
      var cmd, flags, lines, name, prompt_db, ref, x;
      cmd = 'build';
      flags = {
        match: /(?:)/,
        trash_db: true,
